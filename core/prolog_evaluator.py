...
DANGEROUS_PATTERNS = [
    r"\bshell\(",
    r"\bsystem\(",  # OS commands
    r"\bprocess_create\(",
    r"\bwin_exec\(",  # Process spawning
    r"\bopen\(",
    r"\bclose\(",  # File I/O
    r"\bdelete_file\(",
    r"\brename_file\(",  # File manipulation
    r"\bhalt\b",
    r"\babort\b",  # App termination
...
def run_prolog_query(
    ...
    try:
        ...
        if skip_prefix:
            ...
            prefixed_code = re.sub(r"^([a-z][a-zA-Z0-9_]*)(\(|(?=\s*:-))", rf"{prefix}\1\2", code, flags=re.MULTILINE)
            ...
            for pred in set(predicates):
                prefixed_code = re.sub(rf"(?<![a-zA-Z0-9_]){pred}(?=\()", rf"{prefix}{pred}", prefixed_code)
